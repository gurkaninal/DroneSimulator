...
public class TerrainMeshGenerator : MonoBehaviour {
    ...
    public void OnValidate() {
        GenerateMap();
        transform.position = meshVariables.terrainWidth * new Vector3(-.5f, 0, -.5f);
        transform.position += meshVariables.height * Vector3.down;
    }

    public void GenerateMap() {
        NativeArray<Color> _gradientColorArray = new(100, Allocator.Persistent);

        for (int i = 0; i < 100; i++) {
            _gradientColorArray[i] = heightmapGradient.Evaluate(i / 100f);
        }

        HeightMapGenerator heightmapGenerator = new(meshVariables, heightmapVariables, _gradientColorArray);
        heightmapGenerator.Schedule(meshVariables.TotalVerts, 10000).Complete();
        Maps _maps = heightmapGenerator.ReturnAndDispose();

        MeshGenerator meshGenerator = new(meshVariables, _maps);
...
public struct HeightMapGenerator : IJobParallelFor {
    ...
    public readonly Maps ReturnAndDispose() => new(_heightMap, _colMap);

    readonly float OctavedRidgeNoise(float2 pos) {
    ...
    readonly float OctavedSimplexNoise(float2 pos) {
    ...
    readonly float FalloffMap(float2 pos) {
...
public struct TerrainMeshVariables {
    ...
    public float height;
    public readonly int TotalVerts => (terrainMeshDetail + 1) * (terrainMeshDetail + 1);
    public readonly int TotalTriangles => terrainMeshDetail * terrainMeshDetail * 6;
    public readonly float TileEdgeLength => terrainWidth / terrainMeshDetail;
}
[Serializable]
